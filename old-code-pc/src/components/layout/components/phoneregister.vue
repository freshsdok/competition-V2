...
<template>
  <div>
    <div v-if="shurushow == 'shoujihao'">
      <el-form :model="form" label-width="auto" :rules="rules" ref="shoujiRef">
        <el-form-item prop="phonenumber" style="margin-top: 40px">
          <el-input
        ...
        >
        <div class="zhuche">
          <div class="xiangxuanxiang" @click="counterStore.emailregister">
    ...
    <div v-if="shurushow == 'yanzhengma'">
      <div class="yanzhengmatishi">
        验证码已发送至
        {{
          form.phonenumber.substring(0, 3) +
          "****" +
          form.phonenumber.substring(7)
        }}
        , 请输入验证码。
      </div>
      ...
      >
        ...
        <el-button
          ...
          class="denglu"
          style="margin-top: 20px"
          :disabled="!form.msgCode || form.msgCode == ''"
          >验证手机</el-button
        >
        <div class="zhuche">
          <div class="xiangxuanxiang" @click="counterStore.emailregister">
...
// 注册
const shoujiRef = ref(null);
const huoquyanzhengma = () => {
  if (shurushow.value == "shoujihao") {
    shoujiRef.value?.validate((valid) => {
      if (valid) {
        if (form.value.yhxy?.length > 0) {
          console.log(form.value.yhxy);
        } else {
