    ...
    <el-row>
      <div v-if="dataType === 'USERS'">
        ...
        <div class="element-drawer__button">
          <el-button size="default" type="primary" icon="el-icon-plus" @click="onSelectUsers()">添加用户</el-button>
        </div>
      </div>
  ...
  methods: {
    resetTaskForm() {
      ...
      if (this.dataType === 'USERS') {
        ...
        // 回显选择的用户
        if (this.selectedUser.ids.length>0) {
          let userIds = this.selectedUser.ids.join(",");
          selectUserByIds(userIds).then(res => {
            this.selectedUserDate = res.data;
          })
        }
