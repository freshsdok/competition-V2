...
// 添加日期范围添加默认时分秒
export function addDateRangeMin(params, dateRange, propName) {
  let search = params;
  search.params = typeof (search.params) === 'object' && search.params !== null && !Array.isArray(search.params) ? search.params : {};
  dateRange = Array.isArray(dateRange) ? dateRange : [];
  if (typeof (propName) === 'undefined') {
    ...
    search.params['begin' + propName] = dateRange[0] + ' 00:00:00';
    search.params['end' + propName] = dateRange[1] + " 23:59:59";
  }
  return search;
}
...
export function moneyFormat (money) {
  if (Number.isFinite(parseInt(money, 10))) {
    money = Number(money);
 ...
 * @param {*} value 
 */
export function bankAccountFormat (value) {
  if (value === null || value === undefined) {
    return "/"
  } else {
    let acount = "";
    acount = value.replace(/[\s]/g, '').replace(/(\d{4})(?=\d)/g, "$1 ");
    return acount;
  }
}
