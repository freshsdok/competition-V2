...
// js下载方法
export function downloadJS(url, fileName, addName, callback, method = "get") {
  const xhr = new XMLHttpRequest();
  console.log(url, fileName, addName);
  if (addName === "addName") {
...
//  xhr.open("GET", `${url}/${fileName}`, true);
  ...
  xhr.onload = async () => {
    if (xhr.status === 200) {
      if (await blobValidate(xhr.response)) {
        saveAs(xhr.response, fileName || "download");
      } else {
        const result = JSON.parse(await xhr.response.text());
        ElMessage.error(result.msg || "下载文件失败");
      }
    } else {
      // ✅ 处理错误状态
