<template>
  <div class="v-form-designer-container">
    <v-form-designer
      ref="vfDesigner"
...
watch(() => props.form, (val) => {
  if (val && val.content) {
    setTimeout(() => {
      proxy.$nextTick(() => {
        vfDesigner.value?.setFormJson(JSON.parse(val.content));
      })
    },);
  } else {
    setTimeout(() => {
...
<style lang="scss">
  ...
  .v-form-designer-container {
    height: 100%;
    .el-container.main-container .el-container.main-content .left-aside-toggle-bar, .right-aside-toggle-bar{
