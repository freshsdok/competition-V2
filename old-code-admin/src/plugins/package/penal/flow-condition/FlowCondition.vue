<template>
  <div class="panel-tab__content">
    <el-form :model="flowConditionForm" label-width="90px" size="default" @submit.native.prevent>
      ...
      <el-form-item label="表达式" v-if="flowConditionForm.conditionType && flowConditionForm.conditionType === 'expression'" key="express">
        <el-input type="textarea" :rows="3" v-model="flowConditionForm.body" clearable @change="updateFlowCondition" />
      </el-form-item>
  ...
  methods: {
    resetFlowCondition() {
      ...
      if (this.bpmnElementSourceRef && this.bpmnElementSourceRef.default && this.bpmnElementSourceRef.default.id === this.bpmnElement.id) {
        ...
        this.flowConditionForm.type = "condition";
        this.flowConditionForm = { ...this.flowConditionForm, ...conditionExpression };
