...
// 监听content变化，更新modelValue
watch(content, (newVal) => {
  // 防止当modelValue为undefined时发生循环更新
...
const updateReadOnlyStatus = () => {
  if (quillEditorRef.value) {
    ...
    if (quill) {
      // 使用Quill实例的enable方法设置只读状态，false表示只读，true表示可编辑
      quill.enable(!props.readOnly)
    }
  }
}

// 监听readOnly属性变化，更新Quill实例
watch(() => props.readOnly, (newVal) => {
  options.value.readOnly = newVal
  updateReadOnlyStatus()
  
  // Handle paste event listener based on readOnly status
  if (props.type == 'url' && quillEditorRef.value) {
    const quill = quillEditorRef.value.getQuill()
    if (quill) {
      if (newVal) {
        // Remove paste event listener when readOnly
        quill.root.removeEventListener('paste', handlePasteCapture, true)
      } else {
        // Add paste event listener when not readOnly
        quill.root.addEventListener('paste', handlePasteCapture, true)
        
        // Also re-initialize image upload handler
        let toolbar = quill.getModule("toolbar")
        toolbar.addHandler("image", (value) => {
          if (value) {
            proxy.$refs.uploadRef.click()
...
.ql-snow .ql-tooltip[data-mode="link"] input[type="text"]::placeholder,
.ql-snow .ql-tooltip[data-mode="video"] input[type="text"]::placeholder {
  content: "链接地址" !important;
}
/* 针对不同浏览器的占位符文本样式 */
.ql-snow .ql-tooltip[data-mode="link"] input[type="text"]:-ms-input-placeholder,
.ql-snow .ql-tooltip[data-mode="video"] input[type="text"]:-ms-input-placeholder {
  content: "链接地址" !important;
}
.ql-snow .ql-tooltip[data-mode="link"] input[type="text"]::-ms-input-placeholder,
.ql-snow .ql-tooltip[data-mode="video"] input[type="text"]::-ms-input-placeholder {
