  ...
  <div class="app-container">
    <el-tabs
      ...
      <el-tab-pane label="表单信息" name="form">
        <div>
          <template v-if="payOrder || refundOrder">
            <RefundDialog
              ref="refundDialogRef"
              :payOrder="payOrder"
          ...
          <template v-for="(formInfo, index) in processFormList"
                      :key="index">
          <el-card
            class="box-card"
            :header="formInfo.title"
            shadow="never"
            v-if="(formInfo?.formData?.operationType === 'retired' || !formInfo?.formData?.operationType)"
            >
            <!--流程处理表单模块-->
            <el-row >
              <el-col :span="20" :offset="2">
...
const {pay_status,pay_method,pay_mode} = proxy.useDict("pay_status","pay_method","pay_mode");
...
function getProcessDetails(procInsId, taskId) {
  ...
  detailProcess(params).then(res => {
    ...
    payOrder = data?.orderInfos?.payOrder || null
    refundOrder = data?.orderInfos?.refundOrder || null
  })
}
    ...
    proxy.$refs["taskFormRef1"].validate(valid => {
      if (valid) {
        complete(taskForm).then(response => {
          ElMessage.success(response.msg || '已通过');
          goBack();
        });
      }
    });
