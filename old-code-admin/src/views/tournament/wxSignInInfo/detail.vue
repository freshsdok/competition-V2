            ...
            <el-form-item label="指导教师" prop="guidTeacherList">
              <div v-if="form?.guidTeacherList">
                <div v-for="(item,index) in form.guidTeacherList" :key="index">
                  <span style="margin-right: 10px;">姓名：{{ item.instructor || '-' }}</span>
                  <span style="margin-right: 10px;">手机号：{{ item.instructorPhone || '-' }}</span>
                  <span>邮箱：{{ item.instructorEmail || '-' }}</span>
                </div>
              </div>
            </el-form-item>
          </el-col>
...
<script setup name="WxSignInInfoDetail">
