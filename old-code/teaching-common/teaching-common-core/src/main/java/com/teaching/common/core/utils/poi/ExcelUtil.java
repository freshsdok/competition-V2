...
import com.teaching.common.core.exception.UtilException;
...
import com.teaching.common.core.utils.file.ImageUtils;
import com.teaching.common.core.utils.reflect.ReflectUtils;
...
public class ExcelUtil<T>
{
    ...
    public static final String COMPETITION_TRACK_NAME = "赛道名称";

    public static final String COMPETITION_TRACK_PROPERTY = "competitionTrackName";
     ...
     * 必填字段校验最大错误展示数量
     */
    private static final int MAX_REQUIRED_ERROR_COUNT = 5;
    ...
    public List<T> importExcel(InputStream is, int titleNum)
    {
        ...
        {
            log.error("导入Excel验证异常{}", e.getMessage());
            throw e;
        }
        catch (Exception e)
        {
    ...
    public List<T> importExcel(String sheetName, InputStream is, int titleNum) throws Exception
    {
        ...
        if (rows > 0)
        {
            ...
            for (int i = titleNum + 1; i <= rows; i++)
            {
                ...
                for (Map.Entry<Integer, Object[]> entry : fieldsMap.entrySet())
                {
                    ...
                    // 校验必填字段
                    if (validateRequiredField(val, i, attr, errorMessages))
                    {
                        totalErrorCount++;
                    }
                    // 取得类型,并根据对象类型设置值.
        ...
        if (!errorMessages.isEmpty())
        {
            ...
            if (totalErrorCount > MAX_REQUIRED_ERROR_COUNT)
            {
                errorMsg += "（共" + totalErrorCount + "处校验失败）";
            }
            throw new ExcelImportRequiredException(errorMsg);
        }
        return list;
    }

    // 多sheet页excel表单导入
    public List<T>  importExcelForManySheet(InputStream is,Integer titleNum) throws Exception {
        this.type = Type.IMPORT;
        this.wb = WorkbookFactory.create(is);
        ...
        while (sheetIterator.hasNext()) {
            ...
            if (rows > 0) {
                ...
                // 添加赛道cell
                cellMap.put(COMPETITION_TRACK_NAME, cellMap.size());
                // 有数据时才处理 得到类的所有field.
                List<Object[]> fields = this.getFields();
                Map<Integer, Object[]> fieldsMap = new HashMap<Integer, Object[]>();
                for (Object[] objects : fields) {
                ...
                for (int i = titleNum + 1; i <= rows; i++) {
                    ...
                    for (Map.Entry<Integer, Object[]> entry : fieldsMap.entrySet()) {
                        ...
                        if (StringUtils.isNotNull(fieldType)) {
                            ...
                            if(propertyName.equals(COMPETITION_TRACK_PROPERTY)){
                                val = sheet.getSheetName();
                            }
                            ReflectUtils.invokeSetter(entity, propertyName, val);
                        }
                    }
    ...
    public MultipartFile transToMultipartFile(HttpServletResponse response, List<T> list, String sheetName)
    {
        return transToMultipartFile(response, list, sheetName, StringUtils.EMPTY);
    }
    ...
    public MultipartFile transToMultipartFile(HttpServletResponse response, List<T> list, String sheetName, String title)
    {
        response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        response.setCharacterEncoding("utf-8");
        this.init(list, sheetName, title, Type.EXPORT);
        return transToMultipartFile(response,sheetName);
    }
     ...
     * @return 结果
     */
    public void exportExcel(HttpServletResponse response, String sheetName, Class<T> clazz, List<T> list)
    {
        this.init(list, sheetName, "", Type.EXPORT);
        exportExcel(response);
    }

    public MultipartFile exportExcelData(HttpServletResponse response, Map<String, List<?>> listMap,String fileName) {
        try {
            int sheetNum = 0;
            for (String sheetName : listMap.keySet()) {
                List<T> list = (List<T>) listMap.get(sheetName);
                writeSheetToWorkbook(sheetName, list,sheetNum);
                sheetNum++;
            }
//            wb.write(response.getOutputStream());
            // 3. 将工作簿写入 ByteArrayOutputStream
            try (ByteArrayOutputStream bos = new ByteArrayOutputStream()) {
                wb.write(bos);
                wb.close();

                // 4. 创建 MultipartFile
                MultipartFile inMemoryMultipartFile = new InMemoryMultipartFile(
                        bos.toByteArray(),
                        fileName,
    ...
    public void exportExcelByteData(HttpServletResponse response, Map<String, List<?>> listMap) {
        try {
            int sheetNum = 0;
            for (String sheetName : listMap.keySet()) {
                List<T> list = (List<T>) listMap.get(sheetName);
    ...
    private void writeSheetToWorkbook(String sheetName, List<T> list,int sheetNum) {
//        util.init(list, sheetName, "", Type.EXPORT);
        if(CollectionUtils.isNotEmpty(list)){
            Class<T> clazz = (Class<T>) list.get(0).getClass();
            this.clazz = clazz;
        }
        this.setWb(list, sheetName, "", Type.EXPORT, sheetNum);
        writeSheet();
    }
    ...
    public MultipartFile transToMultipartFile(HttpServletResponse response,String sheetName)
    {
        try
        {
            writeSheet();
            ...
            try (ByteArrayOutputStream bos = new ByteArrayOutputStream()) {
                ...
                MultipartFile inMemoryMultipartFile = new InMemoryMultipartFile(
                        bos.toByteArray(),
                        sheetName + "_" + System.currentTimeMillis() + ".xlsx",
                        "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                );
                return inMemoryMultipartFile;
            }
     ...
     * 创建写入数据到Sheet
     */
    public void setWb(List<T> list, String sheetName, String title, Type type,int sheetNum) {
        if (list == null)
        {
            list = new ArrayList<T>();
        }
    ...
    public void createWorkbook(int sheetNum) {
        if(this.wb == null){
     ...
     * @param attr Excel注解
     */
    private boolean validateRequiredField(Object val, int rowNum, Excel attr, List<String> errorMessages)
    {
        if (attr.required())
        {
            if (StringUtils.isNull(val) || StringUtils.isEmpty(val.toString()))
            {
                if (errorMessages.size() < MAX_REQUIRED_ERROR_COUNT)
                {
                    errorMessages.add("第" + (rowNum + 1) + "行" + attr.name() + "不能为空");
                }
                return true;
            }
        }
